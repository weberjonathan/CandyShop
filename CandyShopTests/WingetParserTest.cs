...
namespace CandyShopTests
{
    public class WingetParserTest
    {
        ...
        [Fact]
        public void Constructor_ParsePinnedUnicode_CheckResult()
        {
            // feature:  Occasionally unicode block characters are part of the progress chars
            ...
            string wingetOutput = "\r" +
                ...
                "Name                         ID                                    Version              Quelle Stecknadeltyp\r\n" +
                "------------------------------------------------------------------------------------------------------------\r\n" +
                "Jellyfin Server 10.10.7      Jellyfin.Server                       10.10.7              winget Pinning\r\n" +
                "Spotify                      Spotify.Spotify                       1.2.60.564.gcc6305cb winget Pinning\r\n" +
                "Visual Studio Community 2022 Microsoft.VisualStudio.2022.Community 17.13.6              winget Pinning\r\n" +
                "\r\n";
            ...
            Assert.True(parser.HasTable);
            Assert.Equal(expectedCols, parser.Columns);
            Assert.Equal(3, parser.Items.Length);
            Assert.Equal(5, parser.Items[0].Length);
            Assert.Equal("Jellyfin Server 10.10.7", parser.Items[0][0]);
            Assert.Equal("Spotify", parser.Items[1][0]);
            Assert.Equal("Visual Studio Community 2022", parser.Items[2][0]);
        }

        [Fact]
        public void Constructor_ParsePinnedOnlyUninstalled_CheckResult()
        {
            string wingetOutput = "\r" +
                "   - \r" +
                "   \\ \r" +
            ...
            Assert.True(parser.HasTable);
            Assert.Equal(expectedCols, parser.Columns);
            Assert.Single(parser.Items);
            Assert.Equal(4, parser.Items[0].Length);
            Assert.Equal("Docker Desktop", parser.Items[0][0]);
            Assert.Equal("Docker.DockerDesktop", parser.Items[0][1]);
            Assert.Equal("winget", parser.Items[0][2]);
            Assert.Equal("Pinning", parser.Items[0][3]);
        }

        [Fact]
        public void Constructor_ParsePinned6Cols_CheckResult()
        {
            // feature:  Winget output was observed to contain six columns
            //           This seems like a mistake and violates column layout rules,
            //           but the parser should not abort because the overlapping
            //           columns have no relevant information
            ...
            string wingetOutput = "\r" +
                ...
                "--------------------------------------------------------------------------------\r\n" +
                "NVM for Windows 1.1.12 CoreyButler.NVMforWindows   1.1.12        winget Pinning\r\n" +
                "Epic Games Launcher    EpicGames.EpicGamesLauncher 1.3.142.0     winget Pinning\r\n" +
                "Logitech G HUB         Logitech.GHUB               2024.7.625196 winget Pinning\r\n" +
                "Discord                Discord.Discord             1.0.9147      winget Pinning\r\n";
            ...
            Assert.True(parser.HasTable);
            Assert.Equal(expectedCols, parser.Columns);
            Assert.Equal(4, parser.Items.Length);
            Assert.Equal(6, parser.Items[0].Length);
            Assert.Equal("NVM for Windows 1.1.12", parser.Items[0][0]);
            Assert.Equal("CoreyButler.NVMforWindows", parser.Items[0][1]);
            Assert.Equal("1.1.12", parser.Items[0][2]);
            Assert.Equal("winget", parser.Items[0][3]);
            Assert.Equal("Pinn", parser.Items[0][4]);
            Assert.Equal("ing", parser.Items[0][5]);
        }
        ...
        [Fact]
        public void Trim_ParseInfo_CheckResult()
        {
            string wingetOutput = "\r" +
                "   - " +
                "\r   " +
                ...
                "   | \r" +
                "                                                                                                                        \r" +
                "Gefunden Microsoft System CLR Types for SQL Server 2019 [Microsoft.CLRTypesSQLServer.2019]\r\n" +
                "Version: 15.0.2000.5\r\n" +
                "Herausgeber: Microsoft Corporation\r\n" +
                "Beschreibung: The SQL Server System CLR Types package contains the components implementing the geometry, geography, and hierarchy ID types in SQL Server\r\n" +
                "Lizenz: Proprietary\r\n" +
                "Installationsprogramm:\r\n" +
                "  Installertyp: wix\r\n" +
                "  Installer-Gebietsschema: en-US\r\n" +
                "  Installer-URL: https://download.microsoft.com/download/d/d/1/dd194c5c-d859-49b8-ad64-5cbdcbb9b7bd/SQLSysClrTypes.msi\r\n" +
                "  Sha256-Installer: adb3a406d64f355d8403aa59524c65a8c4be571c6430990cf6b303dc7011d8b5\r\n" +
                "  Unterstützte Offlineverteilung: true\r\n";

            var result = WingetParser.LeftTrimProgressChars(wingetOutput);
            Assert.StartsWith("Gefunden", result);
            Assert.EndsWith("Unterstützte Offlineverteilung: true\r\n", result);
            Assert.Equal(650, result.Length);
        }

        [Fact]
        public void Trim_TableHead_CheckResult()
        {
            string wingetOutput = "\r" +
                "   - \r" +
                "   \\ \r" +
            ...
            const string PARSED_TABLEHEAD =
                "Name                                                               ID                           Version       Verfügbar     Quelle\r\n";

            Assert.Equal(PARSED_TABLEHEAD, WingetParser.LeftTrimProgressChars(wingetOutput));
        }

        [Fact]
        public void Trim_Misformed_CheckResult()
        {
            Assert.Null(WingetParser.LeftTrimProgressChars(null, true));
            Assert.Equal("", WingetParser.LeftTrimProgressChars("", true));
            Assert.Equal("Test without carriage return\r\n", WingetParser.LeftTrimProgressChars("Test without carriage return\r\n", true));
            Assert.Equal("Test string starts with carriage return\r\n", WingetParser.LeftTrimProgressChars("\rTest string starts with carriage return\r\n", true));
            Assert.Equal("Test string without new line", WingetParser.LeftTrimProgressChars("\rTest string without new line", true));
            Assert.Equal("", WingetParser.LeftTrimProgressChars("\r", true));
            Assert.Equal("Test string without new line and carriage return", WingetParser.LeftTrimProgressChars("Test string without new line and carriage return", true));
            Assert.Equal("", WingetParser.LeftTrimProgressChars("\r   - \r   \\ \r   |   █████████████████████████▒▒▒▒▒  86%\r   - \r", true));
            Assert.Throws<PackageManagerException>(() => WingetParser.LeftTrimProgressChars("Non progress indicator\r", true));
        }
    }
}